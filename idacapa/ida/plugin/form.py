...
from idacapa.ida.plugin.icon import QICON
from idacapa.ida.plugin.view import CapaExplorerQtreeView
from idacapa.ida.plugin.hooks import CapaExplorerIdaHooks
from idacapa.ida.plugin.model import CapaExplorerDataModel
from idacapa.ida.plugin.proxy import CapaExplorerRangeProxyModel, CapaExplorerSearchProxyModel
...
class CapaExplorerForm(idaapi.PluginForm):
    ...
    def __init__(self, name):
        ...
        self.rule_path = os.path.join(ida_diskio.idadir("plugins"), "idacapa", "capa-rules")
    ...
    def load_file_menu(self):
        ...
        actions = (
            ...
            ("Export to .tag file?", "Export to .tag file", self.slot_export_to_tag_file),
            ("Export to x64dbg script?", "Export to x64dbg script", self.slot_export_to_x64dbg_script),
            ("Export to cutter/r2gui script?", "Export to cutter/r2gui script", self.slot_export_to_r2_script),
            ("Export capa results...", "Export capa results as pickle file", self.slot_export_pickle),
            ("Import capa results...", "Import capa results as pickle file", self.slot_import_pickle),
        )
    ...
    def slot_export_pickle(self):
        ...
        if not self.doc:
            idaapi.info("No capa results to export.")
            return
        path = idaapi.ask_file(True, os.path.splitext(ida_nalt.get_root_filename())[0] + ".capa", "Choose file")
        if not path:
            return
    ...
    def slot_import_pickle(self):
        ...
        with open(path, "rb") as import_file:
            ...
            try:
                ...
                if self.doc["meta"]["analysis"]["base_address"] != ida_nalt.get_imagebase():
                    if 0 == idaapi.ask_yn(1, "Imagebase is not match! Continue to load ?"):
                        act = False
    ...
    def slot_export_to_tag_file(self):
        ...
        filename = idaapi.ask_file(True, os.path.splitext(ida_nalt.get_root_filename())[0] + ".tag", "Choose file")
        if not filename:
            return
        if os.path.exists(filename) and 1 != idaapi.ask_yn(1, "File already exists. Overwrite?"):
            return
    ...
    def slot_export_to_x64dbg_script(self):
        if not self.doc:
            idaapi.info("No capa results to export.")
            return
        filename = idaapi.ask_file(True, os.path.splitext(ida_nalt.get_root_filename())[0] + ".x64dbg.txt", "Choose file")
        if not filename:
            return
        if os.path.exists(filename) and 1 != idaapi.ask_yn(1, "File already exists. Overwrite?"):
            return
    ...
    def slot_export_to_r2_script(self):
        if not self.doc:
            idaapi.info("No capa results to export.")
            return
        filename = idaapi.ask_file(True, os.path.splitext(ida_nalt.get_root_filename())[0] + ".cutter.r2", "Choose file")
        if not filename:
            return
        if os.path.exists(filename) and 1 != idaapi.ask_yn(1, "File already exists. Overwrite?"):
            return
    ...
    def ida_hook_rename(self, meta, post=False):
        ...
        if not location or not idacapa.ida.helpers.is_func_start(location):
            return
    ...
    def ida_hook_rebase(self, meta, post=False):
        ...
        if post:
            if idaapi.get_imagebase() != meta.get("prev_base", -1):
                idacapa.ida.helpers.inform_user_ida_ui("Running capa analysis again after program rebase")
                self.slot_analyze()
        else:
    ...
    def load_capa_results(self):
        ...
        try:
            ...
            idacapa.ida.helpers.inform_user_ida_ui("Failed to load capa rules from %s" % self.rule_path)
        ...
        try:
            meta = idacapa.ida.helpers.collect_metadata()
            capabilities, counts = capa.main.find_capabilities(rules, extractor, disable_progress=True)
            meta["analysis"].update(counts)
            # meta["analysis"]["base_address"] = ida_nalt.get_imagebase()
        except UserCancelledError:
            logger.info("User cancelled analysis.")
            return False
        except Exception as e:
        ...
        try:
            ...
            if idaapi.get_file_type_name() == "Binary file":
                ...
                idacapa.ida.helpers.inform_user_ida_ui("capa encountered file type warnings during analysis")

            if capa.main.has_file_limitation(rules, capabilities, is_standalone=False):
                idacapa.ida.helpers.inform_user_ida_ui("capa encountered file limitation warnings during analysis")
        except Exception as e:
