...
def is_op_write(insn, op):
    ...
    bits = [ida_idp.CF_CHG1, ida_idp.CF_CHG2, ida_idp.CF_CHG3, ida_idp.CF_CHG4, ida_idp.CF_CHG5, ida_idp.CF_CHG6]
    feature = insn.get_canon_feature()
    return op < ida_ida.UA_MAXOP and (feature & bits[op]) != 0


def is_op_read(insn, op):
    """ Check if an operand is read from (source operand) """
    bits = [ida_idp.CF_USE1, ida_idp.CF_USE2, ida_idp.CF_USE3, ida_idp.CF_USE4, ida_idp.CF_USE5, ida_idp.CF_USE6]
    feature = insn.get_canon_feature()
    return op < ida_ida.UA_MAXOP and (feature & bits[op]) != 0
